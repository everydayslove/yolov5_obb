...
def rotatePoint(xc, yc, xoff, yoff, theta):
...
def cv_boxPoints(rect):
    angleTheta = math.radians(rect[2])
    poly = []
    x = float(rect[0][0])
    y = float(rect[0][1])
    w = float(rect[1][0]/2)
    h = float(rect[1][1]/2)
    poly.append(rotatePoint(x, y, w, h, angleTheta))
    poly.append(rotatePoint(x, y, -w, h, angleTheta))
    poly.append(rotatePoint(x, y, -w, -h, angleTheta))
    poly.append(rotatePoint(x, y, w, -h, angleTheta))
    return poly
...
def scale_labels(img1_shape, labels, img0_shape, ratio_pad=None):
    ...
    for i, label in enumerate(labels):
        ...
        poly = np.array(cv_boxPoints(rect))
        ...
        label = np.array([c_x, c_y, w, h, theta])
...
def rotate_non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False, without_iouthres=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if 0:
            ...
            boxes_xy = boxes_xy.cpu()
            box_whthetas = box_whthetas.cpu()
            rects = longsideformat2poly_batch(boxes_xy, box_whthetas)
        #print("\n\t\t\t\t\ttime: ", time.time() - tt)

        #tt = time.time()
        i = np.array(py_cpu_nms_poly_fast(np.array(rects, dtype=np.float64), scores.cpu().numpy(), iou_thres))
...
def longsideformat2poly(x_c, y_c, longside, shortside, theta_longside):
    ...
    poly = np.double(cv_boxPoints(rect))
    poly.size = 8
    return poly

def longsideformat2poly_batch(boxes_xy, box_whthetas):
    x = boxes_xy[:, 0]
    y = boxes_xy[:, 1]
    length = box_whthetas[:, 0] * 0.5
    width = box_whthetas[:, 1] * 0.5
    theta = np.radians(box_whthetas[:, 2] - 179.9)
    c = np.cos(theta)
    s = np.sin(theta)
    rect = [
        x + length*c - width*s, y + length*s + width*c,
        x + length*c + width*s, y + length*s - width*c,
        x - length*c + width*s, y - length*s - width*c,
        x - length*c - width*s, y - length*s + width*c
    ]
    return np.stack(rect, axis=1)
...
def plot_one_rotated_box(rbox, img, color=None, label=None, line_thickness=None, pi_format=True):
    ...
    poly = np.float32(cv_boxPoints(rect))
    poly = np.int0(poly)
...
def rbox_to_poly(rbox, img, pi_format=True):
    '''
