...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, reload=False):
        self.mode = 'images'
        self.img_size = img_size
    ...
    def image_callback(self, cv_image):
        ...
        if (channels == 3):
            brightness_image = cv2.cvtColor(cv_image, cv2.COLOR_RGB2HSV)[:,:,2]
        else:
        ...
        if abs(err_p) > 0.3:
            return k_p*err_p+k_i*err_i
        return None

    def __next__(self):
        if not self.get_status():
            return None, None, None, None
        retval, self.imgs[0] = self.cap.read()
        if not retval:
            self.stream_status = retval
            return None, None, None, None
        self.count += 1
        img0 = self.imgs.copy()
        ...
        if self.count % 30 == 0:
            exposure_delta = self.image_callback(np.array(img[0]))
            if exposure_delta is not None:
                old_exposure = self.cap.get(cv2.CAP_PROP_EXPOSURE)
                new_exposure = int(old_exposure + exposure_delta * (18 - 3) / 0.088071)
                if new_exposure > 800:
                    new_exposure = 800
                    new_gain = self.cap.get(cv2.CAP_PROP_GAIN) + (new_exposure - 800)//2
                ...
                print("######################### exp:", old_exposure, exposure_delta, new_exposure)
