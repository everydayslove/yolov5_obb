...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        if opt.resume:
            ...
            if not os.path.exists(wdir.parent / f'weights_backup_epoch{start_epoch - 1}'):
        ...
        if rank in [-1, 0]:
            ...
            #     # 测试时使用的是EMA模型
                if os.path.exists(opt.output):
                    ...
                    os.mkdir(opt.output)
                if type(test_path) is not list:
                    run_to_val(model=ema.ema,val_path=test_path, img_size=imgsz, output=opt.output, save_to_txt=True)
                else:
                    for path in test_path:
                        run_to_val(model=ema.ema,val_path=path, img_size=imgsz, output=opt.output, save_to_txt=True)
                validation.run(val_image_path=test_path, detection_path=opt.output, class_names=names, log_dir=log_dir)
            # Write
            # 将测试指标写入result.txt
